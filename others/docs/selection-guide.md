...
│   ├── Need personalized desktop or cross-app personal-assistant workflows? -> MyPCBench
...
│   ├── Need MCP breadth, workflow repair, tool trajectories, governed tools, or safety? -> Toolathlon, TOUCAN, MCP-Flow, Evoflux, Tool Forge, ADK Arena, MCP-AgentBench, CompSkillBench / Compositional Skill Routing, SafeMCP, Attested Tool-Server Admission, MCP-SafetyBench
│   ├── Need MCP server runtime-fault taxonomy? -> MCP Runtime Fault Taxonomy
│   ├── Need MCP server regeneration under API drift? -> DeltaMCP
│   ├── Need MCP source attribution, malicious skill audits, or skill composition risk? -> ProvenanceGuard, Description-Code Inconsistency / DCIChecker, PACT / Argument-Level Provenance, MalSkillBench, SkillHarm, SCR-Bench / Skill Composition Risk
